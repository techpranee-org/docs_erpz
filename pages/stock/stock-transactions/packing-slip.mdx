---
title: Packing Slip
description: A packing slip lists the items in a shipment and is attached to the delivered goods. Learn how to create and manage packing slips in ERPZ.
---
...
From a single Delivery Note, multiple Packing Slips can be created, which is useful when the shipment is packed in different boxes. Each box can have a weight and the number of items it contains. For example, if you're shipping 20 chairs in 4 boxes, each box can contain 5 chairs with different Packing Slips for each box.
...
1. Go to the Packing Slip list and click on **New**.
...
**To Package No**: If there are multiple packages of the same type to be shipped at once, set the **From** and **To Package numbers**. For example, package numbers 1 to 5 in one Packing Slip, then package numbers 6 to 10 in the next Packing Slip, and so on. This will be shown when you print the Packing Slip. Note that this will only work if your shipment has that many quantities of the items.
